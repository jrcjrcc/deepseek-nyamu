...
- **Removed hardcoded Metaso API key.** The built-in default key
  `mk-E384C1DD5E8501BB7EFE27C949AFDE5B` was removed from source. Metaso
  search now requires explicit configuration via `[search] api_key` in
  config.toml or the `METASO_API_KEY` environment variable. Get a free key
  at https://metaso.cn/search-api/playground.
- **Added SVG output sanitization for Mermaid diagrams.** Defense-in-depth
  XSS protection strips `<script>` tags, event handler attributes, and
  `javascript:` URLs from rendered SVGs before injecting into the DOM.
- **Added null-byte and length validation for shell commands.** `exec_shell`
  and `task_gate_run` now reject commands containing null bytes or exceeding
  100 KB, preventing argv injection and DoS abuse.
- **Added critical-danger gate for `task_gate_run`.** Root-filesystem deletion
  (`rm -rf /`), fork bombs, `find / -delete`, remote pipe-to-shell, and `eval`
  are hard-blocked even in YOLO mode. The gate is documented in the base
  system prompt so agents are aware of the restriction.
- **Masked app-server auth token in stderr output.** The full bearer token
  is no longer printed to stderr on startup, preventing accidental leakage
  into log files or process dumps. Set `CODEWHALE_APP_SERVER_SHOW_TOKEN=1`
  to restore full-token output for debugging.
- **Added placeholder API key detection.** Config values starting with
  `YOUR_` (e.g. `YOUR_DEEPSEEK_API_KEY` from config.example.toml) are now
  rejected with a clear error message, preventing accidental use of example
  placeholders.
- **Restricted tool audit log permissions on Unix.** Audit log files written
  via `emit_tool_audit` now use `0o600` (owner read/write only) on Unix
  systems.
- **Added CSRF protection to web admin POST endpoint.** Requires `Origin` or
  `X-Requested-With` header on admin API requests, preventing cross-origin
  form submissions from exploiting cookie-based sessions.

### Changed

- **Docker release notes now recommend `--env-file`** over `-e` for passing
  API keys, reducing the risk of key leakage via process listing.
- **Base system prompt (`base.md`) documents tool safety boundaries.** Added
  a `## 工具安全限制` section listing the five critically dangerous patterns
  that `task_gate_run` always blocks.
...
- **Dream — cross-session memory consolidation subsystem.** A new
  `[dream]` config section enables automated memory consolidation
  at session end. Three gates control triggering: `min_hours` (24),
  `min_sessions` (5), and `enabled` (default `true`). When
  conditions are met, the system initializes `~/.codewhale/memories/`
  and awaits a `/dream` command to run the LLM-powered consolidation
  sub-agent. The sub-agent is restricted to five file I/O tools only
  (`read_file`, `write_file`, `edit_file`, `grep_files`, `list_dir`),
  uses an atomic file lock to prevent concurrent runs, and writes new
  records at the top of each topic file (newest first). Records older
  than 60 days are pruned automatically.
- **Dream memory injection.** Dream topic files are now loaded and
  injected into the system prompt as a `<dream_memories>` block
  alongside the existing `<user_memory>` block, giving the model
  cross-session context on every turn. The block sits below the
  volatile boundary and carries a dedicated `DREAM_MEMORY_GUIDANCE`.
  Total cap: 15 KB across all topic files.
- **Dream file size enforcement.** Hard truncation at load time:
  each topic file is capped at 10 KB (`max_memory_file_size`),
  trimmed from the tail (oldest records) with a `<truncated>` marker.
  The limit is enforced in Rust code, not just prompt guidance.
