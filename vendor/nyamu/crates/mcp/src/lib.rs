//! # MCP (Model Context Protocol) 服务器客户端和运行器
//!
//! 本 crate 实现了 MCP（Model Context Protocol）协议的服务器管理和客户端通信
//! 层，为 AI Agent 提供与外部工具和资源交互的能力。主要组件包括：
//!
//! - **McpServerConfig**：单个 MCP 服务器进程的配置结构，包含服务器名称、
//!   可执行文件路径、命令行参数、环境变量和启用状态。
//!
//! - **McpManager**：MCP 服务器的核心管理器，负责注册、启动、停止和注销
//!   多个 MCP 服务器实例。管理工具和资源的统一注册表，支持：
//!   - 通过 `start_all` 批量启动所有已注册服务器，并返回启动摘要
//!   - 基于工具过滤器 (`ToolFilter`) 的 allow/deny 权限控制
//!   - 工具名称的自动限定（如 `mcp__server__tool` 格式），支持超长名称的
//!     截断和哈希处理
//!   - 完全限定工具名的解析和调用分发
//!
//! - **McpManagedClient 特质**：抽象单个 MCP 服务器的客户端接口，包括
//!   `list_tools`、`call_tool`、`list_resources`、`read_resource` 方法。
//!   内置 `InMemoryMcpClient` 用于测试和默认桩服务。
//!
//! - **Stdio 服务器 (`run_stdio_server`)**：基于 JSON-RPC 2.0 的 MCP 标准
//!   输入/输出服务器，通过 stdin/stdout 接收和响应请求。支持的方法包括
//!   工具管理 (`tools/list`, `tools/call`)、资源管理 (`resources/list`,
//!   `resources/read`)、服务器生命周期管理 (`server/register`,
//!   `server/start`, `server/stop`, `server/unregister`) 等。
//!
//! - **启动状态追踪**：`McpStartupStatus` 和 `McpStartupCompleteEvent` 等类型
//!   用于报告各服务器的启动进度和最终状态。
//!
//! - **辅助函数**：工具名称清洗 (`sanitize_component`)、限定名生成
//!   (`qualify_tool_name`) 和解析 (`parse_qualified_tool_name`)，
//!   以及 URI 中服务器名称的提取 (`parse_server_from_uri`)。
...
impl McpManager {
    /// Create a new empty McpManager with no registered servers.
    pub fn new() -> Self {
        Self {
            configs: HashMap::new(),
            clients: HashMap::new(),
