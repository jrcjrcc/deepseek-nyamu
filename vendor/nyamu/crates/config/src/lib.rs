//! LLM 提供商配置管理模块。
//!
//! 本模块是 `nyamu` 项目的配置核心，负责管理 LLM API 提供商的相关配置项。
//! 包含以下主要组件：
//!
//! - [`ConfigToml`]：TOML 配置文件的运行时表示，支持键值读写、安全覆盖与 secret 解析。
//! - [`ProviderKind`]：枚举类型，定义了 18 个支持的 LLM 提供商（DeepSeek、OpenAI、Nvidia NIM 等）。
//! - [`ProviderConfigToml`] / [`ProvidersToml`]：各提供商的独立配置项（API key、base URL、模型等）。
//! - [`PermissionsToml`]：工具执行权限规则配置，与 `nyamu-execpolicy` 集成。
//! - [`ResolvedRuntimeOptions`]：合并 CLI 参数、环境变量、配置文件和密钥库后的最终运行时选项。
//! - [`ConfigStore`]：配置存储结构，管理全局、用户级和项目级配置的加载、序列化与合并。
//! - 子配置结构：网络策略（[`NetworkPolicyToml`]）、LSP 诊断（[`LspConfigToml`]）、
//!   工作区快照（[`SnapshotsToml`]）、技能注册表（[`SkillsToml`]）、Hook 接收器（[`HookSinksToml`]）。
//!
//! 配置优先级为：CLI 参数 > 配置文件 > secret 存储 > 环境变量。
...
pub use nyamu_execpolicy::ToolAskRule;
use nyamu_secrets::SecretSource;
pub use nyamu_secrets::Secrets;
...
pub struct ConfigToml {
    ...
    /// and `nyamu-tui` can share a single config file.
    pub api_key: Option<String>,
    ...
    /// Native tool catalog controls shared with `nyamu-tui`.
    #[serde(default)]
...
impl ConfigToml {
    /// Merge safe project-level overrides from `$WORKSPACE/.nyamu/config.toml`
    ...
    pub fn resolve_runtime_options(&self, cli: &CliRuntimeOverrides) -> ResolvedRuntimeOptions {
        let no_keyring = Secrets::new(std::sync::Arc::new(
            nyamu_secrets::InMemoryKeyringStore::new(),
        ));
    ...
    pub fn resolve_runtime_options_with_secrets(
        ...
        let (api_key, api_key_source) = if let Some(value) = cli.api_key.clone() {
            ...
            match nyamu_secrets::env_for(provider.as_str()) {
...
///
/// Checks `$WORKSPACE/.nyamu/config.toml` first, falling back to
...
pub fn load_project_config(workspace: &Path) -> Option<ConfigToml> {
    for dir in [nyamu_APP_DIR, LEGACY_APP_DIR] {
...
/// backend (e.g. an [`nyamu_secrets::InMemoryKeyringStore`]) before
/// any resolver runs.
...
// ── nyamu state root (v0.8.44) ──────────────────────────────────
//
// v0.8.44 migrates product-owned app state from ~/.deepseek/ to
// ~/.nyamu/ while keeping ~/.deepseek/ as a compatibility fallback.
// New installs write to ~/.nyamu/. Existing installs with only
// ~/.deepseek/ continue working without data loss.

/// Canonical nyamu app directory name under $HOME.
pub const nyamu_APP_DIR: &str = ".nyamu";

/// Legacy DeepSeek-branded app directory name (compatibility fallback).
pub const LEGACY_APP_DIR: &str = ".deepseek";

/// Resolve the primary nyamu home directory.
///
/// `$nyamu_HOME` takes precedence when set. Otherwise defaults to
/// `$HOME/.nyamu`. This is the write target for new product state.
pub fn nyamu_home() -> Result<PathBuf> {
    if let Ok(val) = std::env::var("nyamu_HOME") {
...
/// Resolve a state subdirectory, preferring the nyamu root if
...
pub fn resolve_state_dir(subdir: &str) -> Result<PathBuf> {
    let primary = nyamu_home()?.join(subdir);
    if primary.exists() {
        return Ok(primary);
    }
...
/// Ensure a state subdirectory exists under the primary nyamu root,
/// creating it if necessary. This is the write-path resolver.
pub fn ensure_state_dir(subdir: &str) -> Result<PathBuf> {
    let dir = nyamu_home()?.join(subdir);
    std::fs::create_dir_all(&dir)
        .with_context(|| format!("failed to create {}/", dir.display()))?;
    Ok(dir)
}

/// Resolve a project-local state subdirectory, preferring `.nyamu/`
/// when it exists, falling back to `.deepseek/` for legacy projects.
///
/// Returns `(true, path)` when the primary `.nyamu/` path is used,
...
pub fn resolve_project_state_dir(workspace: &Path, subdir: &str) -> (bool, PathBuf) {
    let primary = workspace.join(nyamu_APP_DIR).join(subdir);
    if primary.exists() {
        return (true, primary);
    }
...
/// Ensure a project-local state subdirectory exists under `.nyamu/`,
...
pub fn ensure_project_state_dir(workspace: &Path, subdir: &str) -> Result<PathBuf> {
    let dir = workspace.join(nyamu_APP_DIR).join(subdir);
    std::fs::create_dir_all(&dir)
        .with_context(|| format!("failed to create {}/", dir.display()))?;
    Ok(dir)
}
...
pub fn resolve_config_path(explicit: Option<PathBuf>) -> Result<PathBuf> {
    let path = if let Some(path) = explicit {
        path
    } else if let Ok(path) = std::env::var("nyamu_CONFIG_PATH") {
...
pub fn default_config_path() -> Result<PathBuf> {
    // Prefer ~/.nyamu/config.toml when it exists (fresh install or
    // migrated), otherwise fall back to ~/.deepseek/config.toml.
    let primary = nyamu_home()?.join(CONFIG_FILE_NAME);
    if primary.exists() {
        return Ok(primary);
    }
...
impl ConfigMigration {
    pub fn user_notice(&self) -> String {
        format!(
            "Migrated legacy config from {} to {}. Use the .nyamu path for future edits; the .deepseek file remains only as a compatibility fallback.",
...
/// `~/.nyamu/config.toml`. Called on first launch after the config
...
pub fn migrate_config_if_needed() -> Result<Option<ConfigMigration>> {
    ...
    if let Some(parent) = primary.parent() {
        std::fs::create_dir_all(parent).context("failed to create nyamu config directory")?;
    }
    std::fs::copy(&legacy, &primary)
        .context("failed to migrate config from deepseek to nyamu home")?;
    tracing::info!(
...
impl EnvRuntimeOverrides {
    fn load() -> Self {
        Self {
            ...
            deepseek_base_url: std::env::var("nyamu_BASE_URL")
...
mod tests {
    ...
    fn config_store_loads_sibling_permissions_toml() {
        ...
        let dir = std::env::temp_dir().join(format!(
            "nyamu-permissions-schema-{}-{unique}",
            std::process::id()
        ));
    ...
    struct EnvGuard {
        ...
        nyamu_provider: Option<OsString>,
        nyamu_model: Option<OsString>,
        nyamu_base_url: Option<OsString>,
    }
    ...
    impl EnvGuard {
        fn without_deepseek_runtime_overrides() -> Self {
            let guard = Self {
                ...
                nyamu_provider: env::var_os("nyamu_PROVIDER"),
                nyamu_model: env::var_os("nyamu_MODEL"),
                nyamu_base_url: env::var_os("nyamu_BASE_URL"),
            ...
            unsafe {
                ...
                env::remove_var("nyamu_PROVIDER");
                env::remove_var("nyamu_MODEL");
                env::remove_var("nyamu_BASE_URL");
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            ...
            unsafe {
                ...
                Self::restore_var("nyamu_PROVIDER", self.nyamu_provider.take());
                Self::restore_var("nyamu_MODEL", self.nyamu_model.take());
                Self::restore_var("nyamu_BASE_URL", self.nyamu_base_url.take());
    ...
    impl nyamu_secrets::KeyringStore for RecordingSecretsStore {
        fn get(&self, key: &str) -> Result<Option<String>, nyamu_secrets::SecretsError> {
            self.gets.lock().unwrap().push(key.to_string());
            Ok(self.value.clone())
        }

        fn set(&self, _key: &str, _value: &str) -> Result<(), nyamu_secrets::SecretsError> {
            Ok(())
        }

        fn delete(&self, _key: &str) -> Result<(), nyamu_secrets::SecretsError> {
            Ok(())
        }
...
"#;
    ...
    ///      `nyamu config set providers.moonshot.*` CLI invokes.
    ///   3. Switch the active provider through `nyamu_PROVIDER` —
    ...
    fn moonshot_kimi_code_smoke_config_set_then_resolve() -> Result<()> {
        ...
        // Same key paths a user would run via `nyamu config set`.
        ...
        unsafe { env::set_var("nyamu_PROVIDER", "moonshot") };
    ...
    fn app_homes_prefer_home_env_before_platform_home_fallback() {
        ...
        impl Drop for HomeEnvGuard {
            fn drop(&mut self) {
                ...
                unsafe {
                    ...
                    match self.nyamu_home.take() {
                        Some(value) => env::set_var("nyamu_HOME", value),
                        None => env::remove_var("nyamu_HOME"),
                    }
                }
            }
        }
        ...
        let home =
            std::env::temp_dir().join(format!("nyamu-config-home-env-{}", std::process::id()));
        let userprofile = std::env::temp_dir().join(format!(
            "nyamu-config-userprofile-{}",
            std::process::id()
        ));
        let _env = HomeEnvGuard {
            home: env::var_os("HOME"),
            userprofile: env::var_os("USERPROFILE"),
            nyamu_home: env::var_os("nyamu_HOME"),
        };
        // Safety: test-only environment mutation is serialized by env_lock().
        unsafe {
            env::set_var("HOME", &home);
            env::set_var("USERPROFILE", &userprofile);
            env::remove_var("nyamu_HOME");
        }

        assert_eq!(
            nyamu_home().expect("nyamu home"),
            home.join(nyamu_APP_DIR)
        );
        assert_eq!(
        ...
        let explicit = std::env::temp_dir().join(format!(
            "nyamu-config-explicit-home-{}",
            std::process::id()
        ));
        // Safety: test-only environment mutation is serialized by env_lock().
        unsafe {
            env::set_var("nyamu_HOME", &explicit);
        }
        assert_eq!(nyamu_home().expect("explicit home"), explicit);
    }
    ...
    fn migrate_config_reports_copied_legacy_path() {
        ...
        let home = std::env::temp_dir().join(format!(
            "nyamu-config-migration-{}-{unique}",
            std::process::id()
        ));
        let legacy_dir = home.join(LEGACY_APP_DIR);
        let primary_dir = home.join(nyamu_APP_DIR);
        ...
        unsafe {
            ...
            env::set_var("nyamu_HOME", &primary_dir);
        }
        ...
        assert!(notice.contains(".nyamu path for future edits"));
...
"#,
    ...
    /// `nyamu_PROVIDER` is the user-facing env alias for switching the
    /// active provider. It must be honored by the runtime resolver and win
    /// over a root `provider = "deepseek"` config entry.
    #[test]
    fn nyamu_provider_env_switches_active_provider() {
        let _lock = env_lock();
        let _env = EnvGuard::without_deepseek_runtime_overrides();
        // Safety: test-only env mutation guarded by env_lock().
        unsafe {
            env::set_var("nyamu_PROVIDER", "moonshot");
        }
        let mut config = ConfigToml {
            provider: ProviderKind::Deepseek,
            ..ConfigToml::default()
    ...
    /// When both `nyamu_PROVIDER` and the legacy `DEEPSEEK_PROVIDER`
    /// are set, the public alias wins — a user adopting `nyamu_*` in a
    /// fresh shell config is not tripped up by a stale legacy export still
    /// living in their dotfiles.
    #[test]
    fn nyamu_provider_env_wins_over_deepseek_provider_env() {
        let _lock = env_lock();
        let _env = EnvGuard::without_deepseek_runtime_overrides();
        // Safety: test-only env mutation guarded by env_lock().
        unsafe {
    ...
    /// `nyamu_MODEL` is the user-facing env alias for picking a model
    /// against the active provider. It must be honored by the runtime
    /// resolver in place of `DEEPSEEK_MODEL`.
    #[test]
    fn nyamu_model_env_alias_overrides_default_for_active_provider() {
        let _lock = env_lock();
        let _env = EnvGuard::without_deepseek_runtime_overrides();
        // Safety: test-only env mutation guarded by env_lock().
        unsafe {
            env::set_var("nyamu_PROVIDER", "moonshot");
            env::set_var("nyamu_MODEL", "custom-kimi-test-model");
        }
        let config = ConfigToml::default();
    ...
    #[test]
    fn blank_nyamu_model_env_alias_does_not_override_default_for_active_provider() {
        let _lock = env_lock();
        let _env = EnvGuard::without_deepseek_runtime_overrides();
        // Safety: test-only env mutation guarded by env_lock().
        unsafe {
            env::set_var("nyamu_PROVIDER", "moonshot");
            env::set_var("nyamu_MODEL", "   ");
        }
        let config = ConfigToml::default();
    ...
    fn siliconflow_env_overrides_key_base_url_and_model() {
        ...
        // Safety: test-only environment mutation guarded by a module mutex.
        unsafe {
            env::set_var("nyamu_PROVIDER", "siliconflow");
    ...
    fn arcee_env_overrides_key_base_url_and_model() {
        ...
        // Safety: test-only environment mutation guarded by a module mutex.
        unsafe {
            env::set_var("nyamu_PROVIDER", "arcee");
    ...
    fn config_file_resolves_above_env_and_keyring() {
        use nyamu_secrets::KeyringStore;
        let _lock = env_lock();
        let _env = EnvGuard::without_deepseek_runtime_overrides();
        // Safety: env mutation guarded by env_lock().
        unsafe { std::env::set_var("DEEPSEEK_API_KEY", "env-key") };

        let store = std::sync::Arc::new(nyamu_secrets::InMemoryKeyringStore::new());
