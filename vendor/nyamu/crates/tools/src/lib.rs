//! 工具注册表 — 管理工具的定义、注册、调度和执行。
//!
//! 本模块是 Nyamu Agent 的工具系统核心，提供了从工具定义到执行完成的完整链路。
//! 中心入口是 [`ToolRegistry`]，它维护了工具名到处理器和规范的映射，并托管一个
//! [`ToolCallRuntime`] 实例来控制并发执行。
//!
//! 主要组件：
//! - [`ToolRegistry`]：中央注册表，支持注册、列举和调度工具。
//! - [`ToolHandler`] trait：所有工具处理器必须实现的接口，定义工具的 `kind`、
//!   是否变更状态（`is_mutating`）以及实际的执行逻辑（`handle`）。
//! - [`ToolSpec`] / [`ConfiguredToolSpec`]：工具规范，包含名称、输入/输出 JSON Schema、
//!   超时设置和并发支持标志。
//! - [`ToolCall`] / [`ToolInvocation`]：表示一次工具调用请求和验证后的调用上下文，
//!   后者携带调用 ID、工具名和负载供处理器消费。
//! - [`ToolCallRuntime`]：通过读写锁管理并发执行。并行安全的工具获取读锁（可重叠），
//!   串行工具获取写锁（互斥），重入调用跳过加锁以避免死锁。
//! - [`ToolCapability`] / [`ApprovalRequirement`]：工具能力和审批策略枚举。
//! - [`ToolError`] / [`FunctionCallError`]：分别表示输入验证错误和调度层错误。
//! - [`ToolResult`]：统一的工具执行结果，包含内容、成功标志和可选元数据。
//! - 辅助函数（`required_str`、`optional_u64` 等）用于从 JSON 输入中提取参数。
