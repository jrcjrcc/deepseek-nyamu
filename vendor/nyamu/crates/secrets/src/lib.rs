//! API 密钥安全存储 — 多后端密钥管理系统，支持操作系统密钥环和 JSON 文件两种存储方式。
//!
//! 本模块为 Nyamu Agent 提供安全的 API 密钥存储和两级解析能力。核心抽象是
//! [`KeyringStore`] trait，有三种实现：
//!
//! - [`FileKeyringStore`]：默认实现。密钥以 JSON 格式存储在
//!   `~/.nyamu/secrets/secrets.json`，Unix 下文件权限设为 0600（仅所有者可读写），
//!   并会拒绝权限过于宽松的现有文件。首次写入时自动创建目录和文件。
//! - [`DefaultKeyringStore`]：可选的操作系统密钥环后端（macOS Keychain / Windows
//!   Credential Manager / Linux Secret Service），通过环境变量
//!   `nyamu_SECRET_BACKEND=system` 启用。`probe()` 方法可在不写入的情况下检测后端可用性。
//! - [`InMemoryKeyringStore`]：HashMap + Mutex 实现的内存存储，仅用于测试环境。
//!
//! 高层入口 [`Secrets`] 提供两层解析链：密钥存储 → 环境变量。
//! `resolve()` 方法先查询配置的后端存储，若未找到则回退到对应的环境变量。
//! `env_for()` 函数维护了所有提供商名称到环境变量的映射（如 `DEEPSEEK_API_KEY`、
//! `OPENROUTER_API_KEY` 等），支持大小写不敏感的别名匹配。
//! `resolve_with_source()` 还可返回密钥的来源信息，便于调试和审计。

//! Secret storage for nyamu API keys.
//!
...
/// with credentials saved before the nyamu rename. macOS users can verify
...
pub const SECRET_BACKEND_ENV: &str = "nyamu_SECRET_BACKEND";
/// Legacy alias for [`SECRET_BACKEND_ENV`].
pub const LEGACY_SECRET_BACKEND_ENV: &str = "DEEPSEEK_SECRET_BACKEND";
const FILE_BACKEND_LABEL: &str = "file-based (~/.nyamu/secrets/)";
...
/// a JSON file under `~/.nyamu/secrets/` ([`FileKeyringStore`]), or an
...
pub trait KeyringStore: Send + Sync {
    ...
    /// storage backend is active. Examples: `"file-based (~/.nyamu/secrets/)"`,
...
/// at `<home>/.nyamu/secrets/secrets.json` with Unix file mode `0600`
...
impl FileKeyringStore {
    ...
    /// Default path: `<home>/.nyamu/secrets/secrets.json`. Honours
    /// `nyamu_HOME`, then `HOME`, `USERPROFILE`, and finally the platform
    ...
    /// copied into the nyamu store.
    pub fn default_path() -> Result<PathBuf, SecretsError> {
        let primary = default_nyamu_secrets_path()?;
...
fn default_nyamu_secrets_path() -> Result<PathBuf, SecretsError> {
    if let Ok(value) = std::env::var("nyamu_HOME") {
        let trimmed = value.trim();
        if !trimmed.is_empty() {
...
impl Secrets {
    ...
    fn file_backed_default() -> Self {
        let path = FileKeyringStore::default_path()
            .unwrap_or_else(|_| PathBuf::from(".nyamu-secrets.json"));
...
mod tests {
    ...
    #[test]
    fn file_default_path_uses_nyamu_home() {
        let _lock = env_lock();
        clear_known_envs();
        let tmp = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("HOME", tmp.path());
    ...
    #[test]
    fn file_default_path_honors_nyamu_home() {
        let _lock = env_lock();
        clear_known_envs();
        let tmp = tempfile::tempdir().unwrap();
        let custom = tmp.path().join("custom-nyamu");
        let _home = EnvVarGuard::set("HOME", tmp.path());
        let _userprofile = EnvVarGuard::set("USERPROFILE", tmp.path());
        let _nyamu_home = EnvVarGuard::set("nyamu_HOME", &custom);
    ...
    #[test]
    fn file_default_path_migrates_legacy_entries_to_nyamu() {
        let _lock = env_lock();
        clear_known_envs();
        let tmp = tempfile::tempdir().unwrap();
        let _home = EnvVarGuard::set("HOME", tmp.path());
