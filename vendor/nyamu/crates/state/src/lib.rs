//! 持久化状态存储 — 基于 SQLite 和 JSONL 的会话线程、消息、检查点与后台任务管理。
//!
//! 本模块为 Nyamu Agent 提供可靠的本地持久化能力。中心入口是 [`StateStore`]，
//! 它封装了一个 SQLite 数据库和一个追加式 JSONL 会话索引文件，无需外部依赖即可运行。
//!
//! 提供以下数据的 CRUD 操作：
//! - **线程 (Thread)** — 会话元数据、归档和会话索引管理。关键类型 [`ThreadMetadata`]
//!   包含了线程 ID、预览、状态、模型提供商、工作目录、Git 上下文等完整信息。
//! - **消息 (Message)** — 仅追加的消息存储，通过 `parent_entry_id` 字段构建树形
//!   分支结构，支持从任意消息处分叉（fork）创建新分支。
//! - **检查点 (Checkpoint)** — 命名状态快照，用于保存和恢复会话的进度。
//! - **作业 (Job)** — 后台任务跟踪，包含状态（排队/运行中/完成/失败/已取消）和进度信息。
//! - **动态工具 (Dynamic Tool)** — 按线程注册的临时工具，支持运行时增删。
//!
//! 辅助类型包括 `ThreadStatus`、`SessionSource`、`JobStateStatus` 等枚举，以及
//! `ThreadListFilters` 用于线程列表过滤。
