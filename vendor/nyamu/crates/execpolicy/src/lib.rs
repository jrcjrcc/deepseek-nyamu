//! # 执行策略引擎
//!
//! 本 crate 实现了 AI Agent 工具调用的执行策略引擎，负责根据预设规则和
//! 运行时上下文判断某个操作是否需要人工审批。核心组件包括：
//!
//! - **ExecPolicyEngine**：策略引擎的主结构，采用多层规则集进行决策：
//!   - 内置默认层 (`BuiltinDefault`) → Agent 层 (`Agent`) → 用户层 (`User`),
//!     按优先级从低到高排序，高层规则覆盖低层
//!   - 拒绝规则（deny）始终优先于允许规则（trusted），且使用简单前缀匹配
//!   - 允许规则使用基于 Bash 元数 (`BashArityDict`) 的智能前缀匹配，确保
//!     如 `git status` 匹配 `git status -s` 但不匹配 `git push`
//!   - 类型化审批规则 (`ToolAskRule`) 支持按工具名称、命令前缀和文件路径
//!     组合匹配，在 `AskForApproval::Never` 模式下生效
//!
//! - **审批模式 (`AskForApproval`)**：支持多种操作策略：
//!   - `UnlessTrusted`：仅未匹配信任规则时要求审批
//!   - `OnFailure`：执行后仅在失败时请求审批
//!   - `OnRequest`：始终要求审批
//!   - `Reject`：拒绝特定类别（沙箱、规则例外、MCP 诱导）
//!   - `Never`：从不要求审批，但禁止需要审批的命令
//!
//! - **决策输出 (`ExecPolicyDecision`, `ExecApprovalRequirement`)**：
//!   包含是否允许执行、是否需要审批、匹配规则信息以及可选的策略修正提案
//!   （如将未匹配命令的首个 token 加入信任前缀）。
//!
//! - **会话级审批缓存**：`remember_session_approval` / `is_session_approved`
//!   支持在单个会话中记住已批准的决策，避免重复审批。
//!
//! - **路径归一化**：跨平台路径比较，统一处理反斜杠/正斜杠和大小写。
...
use nyamu_protocol::{NetworkPolicyAmendment, NetworkPolicyRuleAction};
