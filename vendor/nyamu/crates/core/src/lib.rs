//! # nyamu 核心引擎
//!
//! 本 crate 是 nyamu 框架的核心运行时组件，负责协调和管理 AI Agent 会话的
//! 完整生命周期。主要功能包括：
//!
//! - **线程管理 (ThreadManager)**：提供会话线程的创建、恢复、分叉、归档、
//!   持久化等全生命周期管理。支持从空白、历史记录或持久化存储初始化线程，
//!   并管理线程元数据与消息历史。
//!
//! - **作业管理 (JobManager)**：管理后台异步作业的生命周期，包括排队、运行、
//!   暂停、恢复、完成、失败和取消等状态转换。支持指数退避重试机制、进度
//!   跟踪和历史记录裁剪，同时提供持久化存储与反序列化能力。
//!
//! - **运行时 (Runtime)**：顶层运行时结构，聚合配置 (`ConfigToml`)、模型
//!   注册表 (`ModelRegistry`)、线程管理器、工具注册表 (`ToolRegistry`)、
//!   MCP 管理器 (`McpManager`)、执行策略引擎 (`ExecPolicyEngine`) 和
//!   Hook 分发器 (`HookDispatcher`) 等核心子系统。对外暴露统一的 API 用于
//!   处理线程请求、提示词请求、工具调用、MCP 服务器启动和作业管理等操作。
//!
//! - **执行策略集成**：在工具调用前通过 `ExecPolicyEngine` 进行权限检查，
//!   支持审批流程（如审批请求帧的构建）、已审批会话缓存等。
//!
//! - **事件与 Hook**：通过 `HookDispatcher` 在各生命周期节点（响应开始/结束、
//!   工具调用阶段、审批生命周期等）发出事件，支持外部监听和日志记录。
//!
//! ## 架构位置
//!
//! 本 crate 依赖 `nyamu_mcp`、`nyamu_execpolicy`、`nyamu_hooks`、
//! `nyamu_protocol`、`nyamu_state`、`nyamu_tools`、`nyamu_config` 等
//! 下层 crate，同时被 `nyamu_app_server` 等上层 crate 所依赖。
...
use nyamu_config::{CliRuntimeOverrides, ConfigToml, ProviderKind};
use nyamu_execpolicy::{
    AskForApproval, ExecApprovalRequirement, ExecPolicyContext, ExecPolicyDecision,
    ExecPolicyEngine,
};
use nyamu_hooks::{HookDispatcher, HookEvent};
use nyamu_mcp::{
...
impl ThreadManager {
    ...
    pub fn spawn_thread_with_history(
        ...
        let thread = Thread {
            ...
            source: match source {
                SessionSource::Interactive => nyamu_protocol::SessionSource::Interactive,
                SessionSource::Resume => nyamu_protocol::SessionSource::Resume,
                SessionSource::Fork => nyamu_protocol::SessionSource::Fork,
                SessionSource::Api => nyamu_protocol::SessionSource::Api,
                SessionSource::Unknown => nyamu_protocol::SessionSource::Unknown,
            },
            name: None,
        };
    ...
    pub async fn mcp_startup(&self) -> McpStartupCompleteEvent {
        ...
        for update in updates {
            let status = match update.status {
                McpManagerStartupStatus::Starting => nyamu_protocol::McpStartupStatus::Starting,
                McpManagerStartupStatus::Ready => nyamu_protocol::McpStartupStatus::Ready,
                McpManagerStartupStatus::Failed { error } => {
                    nyamu_protocol::McpStartupStatus::Failed { error }
                }
                McpManagerStartupStatus::Cancelled => {
                    nyamu_protocol::McpStartupStatus::Cancelled
                }
            };
            self.hooks
                .emit(HookEvent::GenericEventFrame {
                    frame: EventFrame::McpStartupUpdate {
                        update: nyamu_protocol::McpStartupUpdateEvent {
        ...
        self.hooks
            .emit(HookEvent::GenericEventFrame {
                frame: EventFrame::McpStartupComplete {
                    summary: nyamu_protocol::McpStartupCompleteEvent {
                        ready: summary.ready.clone(),
                        failed: summary
                            .failed
                            .iter()
                            .map(|f| nyamu_protocol::McpStartupFailure {
...
fn to_persisted_source(source: &nyamu_protocol::SessionSource) -> SessionSource {
    match source {
        nyamu_protocol::SessionSource::Interactive => SessionSource::Interactive,
        nyamu_protocol::SessionSource::Resume => SessionSource::Resume,
        nyamu_protocol::SessionSource::Fork => SessionSource::Fork,
        nyamu_protocol::SessionSource::Api => SessionSource::Api,
        nyamu_protocol::SessionSource::Unknown => SessionSource::Unknown,
    }
}
...
fn tool_output_value(output: &nyamu_protocol::ToolOutput) -> Value {
