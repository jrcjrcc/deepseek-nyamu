//! # 版本发布与更新检查
//!
//! 本 crate 实现了 nyamu 框架的版本发布检测和自动更新逻辑，负责与 GitHub
//! Releases API 和 CNB 镜像源交互，确定当前版本是否需要更新。主要功能包括：
//!
//! - **发布通道 (`ReleaseChannel`)**：支持稳定版 (`Stable`) 和测试版
//!   (`Beta`) 两个更新通道，通过环境变量控制行为。
//!
//! - **查询源 (`ReleaseQuery`)**：支持多种元数据获取方式：
//!   - `Mirror`：通过自定义镜像基地址和固定版本号获取
//!   - `GitHubLatest`：查询 GitHub 单条最新发布 API
//!   - `GitHubReleaseList`：查询 GitHub 发布列表（用于发现 Beta 版本）
//!
//! - **环境变量覆盖**：通过 `NYAMUWHALE_RELEASE_BASE_URL`、
//!   `NYAMUWHALE_USE_CNB_MIRROR`、`NYAMUWHALE_VERSION` 等环境变量
//!   覆盖默认的发布源和版本号，支持国内网络环境下的 CNB 镜像加速。
//!
//! - **版本比较**：基于语义化版本 (SemVer) 的解析和比较，自动处理 `v` 前缀
//!   和尾部构建信息。`update_is_needed` 整合了稳定版和测试版的不同更新策略，
//!   包括从稳定版切换到同系列的测试版的场景。
//!
//! - **HTTP 客户端**：提供阻塞 (`fetch_release_json_blocking`) 和异步
//!   (`fetch_release_json_async`) 两种请求方式，均带有超时控制和 User-Agent。
//!
//! - **辅助工具**：校验和清单文件名、镜像资产 URL 拼接、网络回退提示信息等。
...
pub const RELEASE_BASE_URL_ENV: &str = "NYAMUWHALE_RELEASE_BASE_URL";

/// Legacy environment variable (alias for [`RELEASE_BASE_URL_ENV`]).
pub const LEGACY_RELEASE_BASE_URL_ENV: &str = "DEEPWHALE_RELEASE_BASE_URL";

/// Environment variable that, when set, enables the CNB mirror for downloads.
pub const CNB_MIRROR_ENV: &str = "NYAMUWHALE_USE_CNB_MIRROR";

/// Environment variable that pins the update target version.
pub const UPDATE_VERSION_ENV: &str = "NYAMUWHALE_VERSION";

/// User-Agent header sent with release metadata requests.
pub const UPDATE_USER_AGENT: &str = "deepwhale-updater";
...
/// CNB mirror if `NYAMUWHALE_USE_CNB_MIRROR` is set. Returns `None` when no
...
/// if neither `NYAMUWHALE_VERSION` nor `DEEPWHALE_VERSION` is set.
        ...
        "GitHub release downloads may be blocked or slow on this network.\n\
         For mainland China, use one of these fallback paths:\n\
           1. Source build from the CNB mirror:\n\
               cargo install --git {CNB_REPO_URL} --tag vX.Y.Z --locked --force\n\
           2. Use a binary asset mirror:\n\
               {RELEASE_BASE_URL_ENV}=https://<mirror>/<release-assets>/ {UPDATE_VERSION_ENV}=X.Y.Z deepwhale update\n\
