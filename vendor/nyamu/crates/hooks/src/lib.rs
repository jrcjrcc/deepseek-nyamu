//! # Hook 分发系统
//!
//! 本 crate 实现了 nyamu 框架的事件钩子（Hook）系统，提供一种松耦合的
//! 观察者模式机制，用于在运行时的各生命周期节点广播事件。主要组件包括：
//!
//! - **HookEvent 枚举**：定义了所有可通过钩子系统发射的事件类型，包括：
//!   - `ResponseStart` / `ResponseDelta` / `ResponseEnd`：AI 响应流的
//!     开始、数据块到达和结束事件
//!   - `ToolLifecycle`：工具调用的生命周期阶段（如调度中、已完成、失败）
//!   - `JobLifecycle`：后台作业的状态转换（排队、运行、完成等）
//!   - `ApprovalLifecycle`：审批请求的生命周期（请求、批准、拒绝）
//!   - `GenericEventFrame`：通用事件帧透传，用于转发协议层事件
//!   所有事件均采用 JSON 序列化，使用 `"type"` 字段作为鉴别器。
//!
//! - **HookSink 特质**：定义事件接收端的抽象接口，支持多种传输方式：
//!   - `StdoutHookSink`：将事件以 JSON 行形式打印到标准输出
//!   - `JsonlHookSink`：追加写入到 JSON Lines 格式的日志文件
//!   - `WebhookHookSink`：通过 HTTP POST 发送到远程端点，内置重试机制
//!   - `UnixSocketHookSink`：通过 Unix 域套接字发送（Unix 平台）
//!
//! - **HookDispatcher**：事件分发器，将所有注册的 `HookSink` 收集起来，
//!   调用 `emit` 时将事件广播给所有接收端。单个 sink 的失败不会影响
//!   其他 sink 的事件接收（静默忽略错误）。
//!
//! - **设计原则**：Hook 系统是尽力而为的观测层，不参与控制流。sink 实现
//!   应避免 panic，仅在真正意外的失败时返回错误。
