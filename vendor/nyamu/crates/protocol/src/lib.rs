//! 工具协议类型定义模块。
//!
//! 本模块定义了 `nyamu` 协议层中所有请求/响应类型及事件帧的结构，
//! 涵盖线程管理、应用配置、工具调用、审批流程和流式事件等场景。
//!
//! 主要类型：
//!
//! - [`Envelope<T>`]：通用消息信封，封装 `request_id` 和可选的 `thread_id`。
//! - [`Thread`] / [`ThreadRequest`] / [`ThreadResponse`]：线程（会话）的创建、启动、恢复、
//!   分叉、列表、读取、归档等生命周期管理。
//! - [`AppRequest`] / [`AppResponse`]：应用级请求，包括能力查询、配置读写、模型列表等。
//! - [`PromptRequest`] / [`PromptResponse`]：简单的文本提示-响应交互。
//! - [`ToolPayload`] / [`ToolOutput`]：工具调用的输入载荷和输出结果，按类型区分
//!   （内置函数、自定义工具、本地 Shell 命令、MCP 工具）。
//! - [`EventFrame`]：完整的事件帧枚举，覆盖响应流增量、工具调用生命周期、
//!   MCP 服务器启动、命令执行、补丁应用、审批请求和错误处理等场景。
//! - [`ReviewDecision`]：用户对审批请求的决策（批准、拒绝、中止等）。
//! - [`ToolKind`]：工具来源分类（内置函数 / MCP）。
//! - [`ResponseChannel`]：响应输出通道（正文文本 / 推理过程）。
//! - [`ExecApprovalRequestEvent`]：命令执行审批请求的详细上下文。
//! - MCP 相关类型：启动状态（[`McpStartupStatus`]）、更新事件（[`McpStartupUpdateEvent`]）、
//!   完成摘要（[`McpStartupCompleteEvent`]）。
//! - [`NetworkApprovalContext`] / [`NetworkPolicyAmendment`]：网络访问审批上下文。
//! - [`ApprovalDecisionRequest`]：用户审批决策的请求体。
//! - [`runtime`]：运行时事件信封子模块。
