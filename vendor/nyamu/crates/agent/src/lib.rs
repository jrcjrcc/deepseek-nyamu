//! Agent 引擎核心与模型注册表 — 管理模型元数据并将用户请求的模型名称解析为具体提供商模型。
//!
//! 本模块是 Nyamu Agent 的模型管理层，负责维护所有支持的模型信息并将模糊的用户请求
//! 解析为特定提供商的正式模型标识。中心结构是 [`ModelRegistry`]。
//!
//! 主要组件：
//! - [`ModelRegistry`]：模型注册表，内置了 DeepSeek、Nvidia NIM、OpenRouter、
//!   SiliconFlow、AtlasCloud、Xiaomi Mimo、Arcee、Volcengine、SGLang、vLLM、
//!   Ollama、HuggingFace 等多个提供商的模型条目与别名映射。`Default` 实现预填充
//!   了所有内置模型，`resolve()` 方法实现了多级回退解析逻辑。
//! - [`ModelInfo`]：单个模型的元数据结构，包含 ID、提供商、别名列表、是否支持
//!   工具调用和推理能力标志。
//! - [`ModelResolution`]：模型解析结果，记录了原始请求名称、解析后的具体模型、
//!   是否使用了回退机制以及完整的解析链。
//! - [`ModelFamily`]：高级模型系列枚举，用于客户端跨提供商的身份识别（如 DeepSeek、
//!   Anthropic、OpenAI、Google、Meta、Mistral、Qwen 等）。
//! - [`model_family()`] 函数：根据模型 ID 字符串分类其所属的模型家族。
//!
//! 解析优先级：提供商提示匹配 → 别名索引查找 → 提供商默认模型 → 全局默认模型。
//! 对于 Ollama 提供商，请求名称直接透传，以支持任意本地模型标签。

use std::collections::HashMap;

use nyamu_config::ProviderKind;
