...
- **Removed hardcoded Metaso API key.** The built-in default key
  ...
  config.toml or the `METASO_API_KEY` environment variable.
...
- **Added critical-danger gate for `task_gate_run`.** Root-filesystem deletion
  ...
  are hard-blocked even in YOLO mode. Documented in the base system prompt.
- **Masked app-server auth token in stderr output.** The full bearer token
  is no longer printed to stderr on startup. Set `CODEWHALE_APP_SERVER_SHOW_TOKEN=1`
  to restore for debugging.
- **Added placeholder API key detection.** Config values starting with
  `YOUR_` are now rejected with a clear error message.
- **Restricted tool audit log permissions on Unix.** Audit log files now use
  `0o600` (owner read/write only) on Unix systems.
- **Added CSRF protection to web admin POST endpoint.** Requires `Origin` or
  `X-Requested-With` header on admin API requests.
