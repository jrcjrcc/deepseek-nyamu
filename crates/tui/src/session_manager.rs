...
/// Maximum total size (in bytes) of all session JSON files combined.
/// When exceeded, the oldest sessions are deleted during cleanup.
const MAX_SESSION_STORAGE_BYTES: u64 = 50 * 1024 * 1024; // 50 MB
...
impl SessionManager {
    ...
    pub fn save_session(&self, session: &SavedSession) -> std::io::Result<PathBuf> {
        ...
        // Skip writing empty sessions (0 messages) to avoid disk bloat (#6).
    ...
    /// and keep total session-file size under `MAX_SESSION_STORAGE_BYTES`.
    pub fn cleanup_old_sessions(&self) -> std::io::Result<()> {
        ...
        for session in &sessions {
            if let Ok(path) = self.validated_session_path(&session.id) {
        ...
        if total_size > MAX_SESSION_STORAGE_BYTES {
            // Delete oldest sessions (end of list, since sorted newest-first)
            // until total size is within the limit.
