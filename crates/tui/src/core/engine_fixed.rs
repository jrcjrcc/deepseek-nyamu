...
pub struct EngineConfig {
    ...
    /// Whether the Dream memory consolidation feature is enabled.
    /// When `true`, Dream topic files are loaded and injected into
    /// the system prompt as a `<dream_memories>` block.
    pub dream_enabled: bool,
    /// Resolved path to the Dream memory directory
    /// (`~/.codewhale/memories/`). Only consulted when
    /// `dream_enabled` is `true`.
    pub dream_memory_dir: PathBuf,
    /// Maximum per-file size for Dream topic files, in bytes.
    /// Files exceeding this are truncated at load time.
    pub dream_max_file_size: usize,
...
impl Default for EngineConfig {
    fn default() -> Self {
        Self {
            ...
            dream_enabled: false,
            dream_memory_dir: PathBuf::from("~/.codewhale/memories"),
            dream_max_file_size: 10 * 1024,
            vision_config: None,
...
pub struct EngineHandle {
    ...
    pub rx_event: Arc<RwLock<mpsc::UnboundedReceiver<Event>>>,
...
impl Engine {
    ...
    pub fn new(config: EngineConfig, api_config: &Config) -> (Self, EngineHandle) {
        ...
        let dream_memory_block = crate::dream_memory::compose_block(
            config.dream_enabled,
            &config.dream_memory_dir,
            config.dream_max_file_size,
        );
        ...
        let system_prompt =
            prompts::system_prompt_for_mode_with_context_skills_session_and_approval(
                ...
                prompts::PromptSessionContext {
                    ...
                    dream_memory_block: dream_memory_block.as_deref(),
        ...
        while let Some(op) = self.rx_op.recv().await {
            match op {
                ...
                Op::ListSubAgents => {
                    ...
                    let _ = self.tx_event.send(Event::AgentList { agents });
                }
                Op::ChangeMode { mode } => {
                    let _ = self
                        .tx_event
    ...
    fn refresh_system_prompt(&mut self, mode: AppMode) {
        ...
        let dream_memory_block = crate::dream_memory::compose_block(
            ...
            &self.config.dream_memory_dir,
            self.config.dream_max_file_size,
        );
