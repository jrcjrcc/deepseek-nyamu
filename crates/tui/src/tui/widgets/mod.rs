...
pub(crate) fn slash_completion_hints(
    ...
    if completing_skill_arg.is_none() {
        for name in commands::all_command_names_matching(prefix, workspace) {
            // 只显示匹配当前前缀的命令
            if (input_prefix_char == "&") != name.starts_with('&') {
                continue;
            }
            seen.insert(name.clone());
            let command_key = name.trim_start_matches('/').trim_start_matches('&');
    ...
    if completing_skill_arg.is_none() {
        for cmd in commands::COMMANDS {
            if !matches_input_prefix(cmd) {
                continue;
            }
            let name = format!("{}{}", commands::cmd_display_prefix(cmd), cmd.name);
    ...
    let rank = |entry: &SlashMenuEntry| -> u8 {
        ...
        let command_key = entry.name.trim_start_matches('/').trim_start_matches('&');
...
/// and alias hints.
/// & 命令的中文短描述
fn amp_desc(name: &str) -> &'static str {
    match name {
        "init" => "交互式项目初始化",
        "glance" => "项目速览",
        "search" => "文件内容搜索",
        ...
        "implement" => "导入代码并自动适配",
        "refactor" => "智能重构与去重",
        "security-scan" | "securityscan" => "安全漏洞扫描",
        "predict-issues" | "predictissues" => "预测潜在问题",
        "remove-comments" | "removecomments" => "清理无用注释",
        "fix-imports" | "fiximports" => "修复断裂的导入",
        "find-todos" | "findtodos" => "查找 TODO 标记",
        "create-todos" | "createtodos" => "添加上下文 TODO",
        "fix-todos" | "fixtodos" => "实现 TODO 修复",
        "understand" => "分析项目架构与设计",
        "explain" | "explain-like-senior" | "explainlikesenior" => "资深工程师级代码讲解",
        "contributing" => "贡献就绪度分析",
        "make-it-pretty" | "makeitpretty" => "提升代码可读性",
        "session-start" | "sessionstart" => "开始记录会话",
        ...
        "dream" | "meng" | "记忆整合" => "记忆整合（四阶段整合记忆文件）",
        "tips" => "注入 tips.md 记忆技巧到上下文",
        "traps" => "注入 traps.md 踩坑记录到上下文",
        "update" => "分析会话并更新项目指令文件",
        _ => "自定义命令",
    }
}
...
fn push_command_entry(
    ...
    let (description, alias_hint) = if let Some(info) = commands::get_command_info(command_key) {
        // & 命令绕过 localization，用内置短描述
        if info.usage.starts_with('&') {
            let desc = amp_desc(info.name);
            ...
            return entries.push(SlashMenuEntry {
                name: name.to_string(),
                description: format!("{desc}{aliases}"),
                is_skill: false,
                alias_hint: None,
            });
        }
        ...
        let aliases = info
            .aliases
            .iter()
            .map(|a| format!("{}{}", commands::cmd_display_prefix(info), a))
            .collect::<Vec<_>>()
            .join(", ");
...
mod tests {
    ...
    fn approval_selected_destructive_option_uses_contrasting_highlight() {
        ...
        assert!(
            highlighted_cells >= 4,
            "selected destructive option should render visible selection/white text"
        );
    }
