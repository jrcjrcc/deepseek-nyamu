...
pub(crate) fn open_context_menu(app: &mut App, mouse: MouseEvent) {
    ...
    app.view_stack.push(ContextMenuView::new(
        ...
        mouse.row,
        app.ui_theme,
        app.tr(MessageId::CtxMenuTitle).to_string(),
    ));
    app.needs_redraw = true;
}
...
pub(crate) fn build_context_menu_entries(app: &App, mouse: MouseEvent) -> Vec<ContextMenuEntry> {
    ...
    if selection_has_content(app) {
        entries.push(ContextMenuEntry {
            label: app.tr(MessageId::CtxCopySelection).to_string(),
            description: app.tr(MessageId::CtxCopySelectionDesc).to_string(),
            action: ContextMenuAction::CopySelection,
        });
        entries.push(ContextMenuEntry {
            label: app.tr(MessageId::CtxOpenSelection).to_string(),
            description: app.tr(MessageId::CtxOpenSelectionDesc).to_string(),
            action: ContextMenuAction::OpenSelection,
        });
        entries.push(ContextMenuEntry {
            label: app.tr(MessageId::CtxClearSelection).to_string(),
    ...
    if let Some(filtered_cell_index) = transcript_cell_index_from_mouse(app, mouse) {
        ...
        entries.push(ContextMenuEntry {
            label: app.tr(MessageId::CtxOpenDetails).to_string(),
            description: target,
            action: ContextMenuAction::OpenDetails { cell_index },
        });
        entries.push(ContextMenuEntry {
            label: app.tr(MessageId::CtxCopyMessage).to_string(),
            description: app.tr(MessageId::CtxCopyMessageDesc).to_string(),
            action: ContextMenuAction::CopyCell { cell_index },
        });
        entries.push(ContextMenuEntry {
            label: app.tr(MessageId::CtxOpenInEditor).to_string(),
            description: app.tr(MessageId::CtxOpenInEditorDesc).to_string(),
            action: ContextMenuAction::OpenFileAtLine { cell_index },
        });
        // Hide/show cell toggle.
        if app.collapsed_cells.contains(&cell_index) {
            entries.push(ContextMenuEntry {
                label: app.tr(MessageId::CtxShowCell).to_string(),
                description: app.tr(MessageId::CtxShowCellDesc).to_string(),
                action: ContextMenuAction::ShowCell { cell_index },
            });
        } else {
            entries.push(ContextMenuEntry {
                label: app.tr(MessageId::CtxHideCell).to_string(),
                description: app.tr(MessageId::CtxHideCellDesc).to_string(),
    ...
    entries.push(ContextMenuEntry {
        label: app.tr(MessageId::CtxPaste).to_string(),
        description: app.tr(MessageId::CtxPasteDesc).to_string(),
        action: ContextMenuAction::Paste,
    });
    entries.push(ContextMenuEntry {
        label: app.tr(MessageId::CtxCommandPalette).to_string(),
        description: app.tr(MessageId::CtxCommandPaletteDesc).to_string(),
        action: ContextMenuAction::OpenCommandPalette,
    });
    entries.push(ContextMenuEntry {
        label: app.tr(MessageId::CtxContextInspector).to_string(),
        description: app.tr(MessageId::CtxContextInspectorDesc).to_string(),
        action: ContextMenuAction::OpenContextInspector,
    });
    entries.push(ContextMenuEntry {
        label: app.tr(MessageId::CtxHelp).to_string(),
        description: app.tr(MessageId::CtxHelpDesc).to_string(),
