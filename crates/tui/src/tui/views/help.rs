...
pub(crate) fn command_descriptor(name: &str) -> String {
    match name {
        ...
        "fix-imports" | "fiximports" => "修复断裂的导入".into(),
        "find-todos" | "findtodos" => "查找 TODO 标记".into(),
        "create-todos" | "createtodos" => "添加上下文 TODO".into(),
        "fix-todos" | "fixtodos" => "实现 TODO 修复".into(),
        "understand" => "分析项目架构与设计".into(),
        "explain" | "explain-like-senior" | "explainlikesenior" => "资深工程师级代码讲解".into(),
        "contributing" => "贡献就绪度分析".into(),
        ...
        "traps" => "注入 traps.md 踩坑记录到上下文".into(),
        "update" => "分析会话并更新项目指令文件".into(),
        _ => format!("&{name} 命令"),
    }
}
...
fn build_entries(locale: Locale) -> Vec<HelpEntry> {
    ...
    for command in commands::COMMANDS {
        ...
        // 改用内置短描述
        let localized = if prefix == "&" {
            command_descriptor(command.name)
        } else {
            command.description_for(locale).to_string()
        };
