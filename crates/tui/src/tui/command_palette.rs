...
pub fn build_entries(
    ...
    for command in commands::COMMANDS {
        ...
        let mut description = if prefix == "&" {
            ...
            command.palette_description_for(locale)
        };
        ...
        let action = if command_runs_directly(command.name) {
            CommandPaletteAction::ExecuteCommand {
                command: format!("{prefix}{}", command.name),
            }
        } else {
        ...
        entries.push(CommandPaletteEntry {
            ...
            label: format!("{prefix}{}", command.name),
            description,
...
fn command_runs_directly(_name: &str) -> bool {
    false
}

/// Chinese descriptions for built-in tools. Falls back to English for any
/// tool not listed here.
fn tool_zh_description(name: &str) -> Option<String> {
    let desc = match name {
        "read_file" => "读取文件内容",
        "list_dir" => "列出目录条目",
        "view_image" => "查看图片文件",
        ...
        "exec_shell_wait" => "等待 shell 命令完成",
        "exec_shell_interact" => "交互式运行 shell",
        "grep_files" => "在文件中搜索文本",
        "file_search" => "按名称搜索文件",
        "web_search" => "搜索网络",
        "fetch_url" => "获取 URL 内容",
        ...
        "task_gate_run" => "运行任务门禁",
        "pr_attempt_record" => "记录 PR 尝试",
        "github_issue_context" => "获取 GitHub Issue 上下文",
        "github_pr_context" => "获取 GitHub PR 上下文",
        "github_comment" => "发表 GitHub 评论",
        "checklist_write" => "创建或更新检查清单",
        "checklist_add" => "向清单添加条目",
        "checklist_update" => "更新清单条目状态",
        "web.run" => "自动浏览网页",
        "plan" => "创建执行计划",
        _ => return None,
    };
...
impl CommandPaletteView {
    ...
    /// Flat row model matching HelpView's pattern. Each Section or Entry
    /// takes exactly one row, so scroll-offset math is trivial.
    fn render_rows(&self) -> Vec<PaletteRenderRow> {
        let mut rows = Vec::new();
        let mut active: Option<PaletteSection> = None;
        for (slot, &idx) in self.filtered.iter().enumerate() {
            let sec = self.entries[idx].section;
            if active != Some(sec) {
                rows.push(PaletteRenderRow::Section(sec));
                active = Some(sec);
            }
            rows.push(PaletteRenderRow::Entry { slot });
        }
        rows
    }

    fn selected_render_row(rows: &[PaletteRenderRow], selected: usize) -> usize {
        rows.iter()
            .position(|r| matches!(r, PaletteRenderRow::Entry { slot } if *slot == selected))
            .unwrap_or(0)
    }
    ...
    fn visible_row_start(rows: &[PaletteRenderRow], selected: usize, visible_budget: usize) -> usize {
        ...
        let sel_row = Self::selected_render_row(rows, selected);
        let half = visible_budget / 2;
        if sel_row <= half {
            0
        } else if sel_row + half >= rows.len() {
            rows.len().saturating_sub(visible_budget)
        } else {
            sel_row.saturating_sub(half)
        }
    }
...
impl ModalView for CommandPaletteView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.filtered.is_empty() {
            ...
            let visible_budget = visible; // line budget = header-free height
            ...
            for row in rows.iter().skip(row_start).take(visible_budget) {
                match *row {
                    PaletteRenderRow::Section(sec) => {
                        ...
                        lines.push(Self::format_section_label(sec, count));
                    }
                    PaletteRenderRow::Entry { slot } => {
                        let is_selected = slot == self.selected;
                        let entry = &self.entries[self.filtered[slot]];
                        ...
                        let desc_cap = popup_width as usize - (label_width + 6);
                        ...
                        let line_text = format!("{cursor}{:<label_width$}  {desc}", entry.label);
