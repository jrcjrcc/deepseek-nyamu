...
fn work_panel_lines(
    ...
    if lines.is_empty() {
        lines.push(Line::from(Span::styled(
            work_panel_empty_hint(app, content_width),
...
#[must_use]
fn work_panel_empty_hint(app: &App, content_width: usize) -> String {
    truncate_line_to_width(app.tr(MessageId::SidebarNoActiveWork), content_width)
}
...
fn task_panel_lines(app: &App, content_width: usize, max_rows: usize) -> Vec<Line<'static>> {
    ...
    if !background_rows.is_empty() && lines.len() < max_rows {
        ...
        let header = app.tr(MessageId::SidebarBackgroundHeader);
        let run_label = app.tr(MessageId::SidebarBackgroundRunning);
        let done_label = app.tr(MessageId::SidebarBackgroundCompleted);
        let label = if running == 0 {
            format!("{header}: {done} {done_label}")
        } else if done == 0 {
            format!("{header}: {running} {run_label}")
        } else {
            format!("{header}: {running} {run_label}, {done} {done_label}")
        };
        lines.push(Line::from(Span::styled(
            label,
    ...
    if lines.len() < max_rows {
        ...
        if !recent_rows.is_empty() {
            push_sidebar_label(&mut lines, "最近工具", palette::TEXT_DIM);
    ...
    {
        lines.push(Line::from(Span::styled(
            app.tr(MessageId::SidebarNoLiveTools),
            Style::default().fg(palette::TEXT_MUTED),
        )));
    }
...
fn render_sidebar_subagents(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    render_sidebar_section(f, area, "代理", lines, Vec::new(), app);
}
    ...
    let mut rows: Vec<SidebarAgentRow> = app
        ...
        .map(|agent| {
            ...
            SidebarAgentRow {
                ...
                status: subagent_status_text(&agent.status, app),
                progress,
...
fn subagent_status_text(status: &SubAgentStatus, app: &App) -> String {
    match status {
        SubAgentStatus::Running => app.tr(MessageId::SidebarAgentRunning).to_string(),
        SubAgentStatus::Completed => app.tr(MessageId::SidebarAgentDone).to_string(),
        SubAgentStatus::Interrupted(_) => app.tr(MessageId::SidebarAgentInterrupted).to_string(),
        SubAgentStatus::Failed(_) => app.tr(MessageId::SidebarAgentFailed).to_string(),
        SubAgentStatus::Cancelled => app.tr(MessageId::SidebarAgentCancelled).to_string(),
    }
}
...
fn render_context_panel(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    render_sidebar_section(f, area, "Session（会话）", lines, Vec::new(), app);
}
...
mod tests {
    ...
    fn work_panel_empty_hint_stays_quiet_and_truncates() {
        let hint = work_panel_empty_hint(&create_test_app(), 10);
        assert!(
            hint.chars().count() <= 10,
            "hint width {} > 10: {hint:?}",
            hint.chars().count()
