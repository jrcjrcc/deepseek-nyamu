...
async fn run_event_loop(
    ...
    loop {
        ...
        if done && let Ok(Some(_hint)) = version_check.take().unwrap().await {
            // Version check suppressed — hint is not shown in the footer.
            // app.version_hint = Some(_hint);
        }
            ...
            match key.code {
                ...
                KeyCode::Char('1') if key.modifiers.contains(KeyModifiers::ALT) => {
                    if key.modifiers.contains(KeyModifiers::CONTROL) {
                        ...
                        app.status_message = Some("侧栏焦点：工作".to_string());
                    } else {
                ...
                KeyCode::Char('2') if key.modifiers.contains(KeyModifiers::ALT) => {
                    if key.modifiers.contains(KeyModifiers::CONTROL) {
                        ...
                        app.status_message = Some("侧栏焦点：任务".to_string());
                    } else {
                ...
                KeyCode::Char('3') if key.modifiers.contains(KeyModifiers::ALT) => {
                    if key.modifiers.contains(KeyModifiers::CONTROL) {
                        ...
                        app.status_message = Some("侧栏焦点：代理".to_string());
                    } else {
                    ...
                    if key.modifiers.contains(KeyModifiers::ALT) =>
                {
                    ...
                    app.status_message = Some("侧栏焦点：上下文".to_string());
                    continue;
                }
                KeyCode::Char(')') if key.modifiers.contains(KeyModifiers::ALT) => {
                    app.set_sidebar_focus(SidebarFocus::Auto);
                    app.status_message = Some("侧栏焦点：自动".to_string());
                    continue;
                }
...
fn apply_alt_0_shortcut(app: &mut App, modifiers: KeyModifiers) {
    if modifiers.contains(KeyModifiers::CONTROL) {
        if app.sidebar_focus == SidebarFocus::Hidden {
            ...
            app.status_message = Some("侧栏已隐藏".to_string());
        }
    } else {
        app.set_sidebar_focus(SidebarFocus::Auto);
        app.status_message = Some("侧栏焦点：自动".to_string());
