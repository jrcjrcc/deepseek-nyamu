...
pub fn handle_paste_burst_key(app: &mut App, key: &KeyEvent, now: Instant) -> bool {
    ...
    match key.code {
        ...
        KeyCode::Char(c) if !has_ctrl_alt_or_super => {
            if !c.is_ascii() {
                // IME 输入（中文、日文、韩文等）必须绕过 paste_burst 检测——
                // IME 提交时短时间内的连续字符事件容易触发 paste_burst 阈值
                // （3 字符 / 8ms），导致字符被缓冲在 paste buffer 中而不显示，
                // 表现为输入法"失效"。先 flush 任何 pending 的 paste buffer，
                // 再将字符直接插入 composer。
