...
impl ViewImageCell {
    ...
    pub fn lines_with_motion(&self, width: u16, low_motion: bool) -> Vec<Line<'static>> {
        ...
        lines.extend(render_compact_kv("路径", &path, tool_value_style(), width));
        lines
    }
}
    ...
    if let Ok(json) = serde_json::from_str::<Value>(output) {
        if let Some(obj) = json.as_object() {
            if let Some(error) = obj.get("error").or(obj.get("status_msg")) {
                return format!("错误: {}", summarize_inline_value(error, 120, false));
            }
...
fn render_thinking(
    ...
    // Reasoning body lines are italic only — no background tint.
    // The dashed rail (╎) and italic styling distinguish thinking from
    // regular content without creating a visible colored block.
...
fn render_tool_output_mode(
    ...
    render_preserved_output_mode(output, width, line_limit, mode, "结果")
}
...
fn render_exec_output_mode(
    ...
    render_preserved_output_mode(output, width, line_limit, mode, "输出")
}
...
fn render_tool_header_with_family_and_summary(
    ...
    let state_owned: String = if state == "运行中"
...
fn thinking_status_label(state: ThinkingVisualState) -> &'static str {
    match state {
        ThinkingVisualState::Live => "思考中",
        ThinkingVisualState::Done => "已完成",
