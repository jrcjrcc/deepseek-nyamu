...
const SYSTEM_LAYER_MARKERS: &[(&str, &str, PromptLayerKind)] = &[
    ...
    ("环境信息", "## Environment", PromptLayerKind::Static),
    ("技能列表", "## Skills", PromptLayerKind::Static),
    (
        "上下文管理",
        "## Context Management",
        PromptLayerKind::Static,
...
fn push_system_prompt_structure(out: &mut String, app: &App) {
    ...
    match &app.system_prompt {
        Some(SystemPrompt::Blocks(blocks)) => {
            ...
            let _ = writeln!(
                out,
                "  稳定前缀: {stable_count} 块, ~{stable_tokens} tokens  [可缓存]"
            );
            if let Some(block) = working_block {
                let _ = writeln!(
                    out,
                    "  易变工作集: 1 块, ~{working_tokens} tokens  [每轮变化]"
                );
                let _ = writeln!(
                    out,
                    "    首行: {}",
                    block.text.lines().next().unwrap_or("(空)")
                );
            } else {
...
fn push_references(out: &mut String, references: &[SessionContextReference]) {
    ...
    for record in references {
        ...
        if rendered >= MAX_REFERENCE_ROWS {
            ...
            if remaining > 0 {
                let _ = writeln!(out, "- ... 还有 {remaining} 条引用");
            }
            break;
        }
...
fn push_tools(out: &mut String, app: &App) {
    ...
    {
        let location = format!("卡片 {cell_idx}");
