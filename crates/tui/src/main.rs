...
/// On Windows, if we're in a plain conhost (not Windows Terminal, not a
/// modern third-party terminal, not a pipe/redirection), try to relocate
/// ourselves into Windows Terminal for proper mouse/scrolling support.
/// Falls back silently if `wt.exe` is unavailable.
#[cfg(windows)]
fn relocate_to_windows_terminal() {
    ...
    // In a known third-party terminal (VS Code, ConEmu, etc.).
    if std::env::var("TERM_PROGRAM").is_ok() || std::env::var("ConEmuPID").is_ok() {
        return;
    }
    // Opt-out env var.
    if std::env::var("CODEWHALE_NO_WT_RELOCATE").is_ok() {
        return;
    }
    // stdin is piped/redirected (e.g. `echo foo | codewhale`).
    ...
    // wt.exe not available — stay in conhost.
}
...
enum Commands {
    ...
    Login {
        ...
        /// Provider to log into (default: deepseek)
        #[arg(long, value_enum)]
    ...
    // ── CLI direct commands (merged from cli crate) ──────────────────
    /// Manage authentication credentials and provider mode (merged from CLI).
    Auth(cli_commands::AuthArgs),
    /// Run MCP server mode over stdio (merged from CLI).
    McpServer,
    /// Read/write/list config values (merged from CLI).
    Config(cli_commands::ConfigArgs),
    /// Resolve or list available models across providers (merged from CLI).
    Model(cli_commands::ModelArgs),
    /// Manage thread/session metadata (merged from CLI).
    Thread(cli_commands::ThreadArgs),
    /// Run the app-server transport (merged from CLI).
    AppServer(cli_commands::AppServerArgs),
    /// Print a usage rollup from the audit log and session store (merged from CLI).
    Metrics(cli_commands::MetricsArgs),
    /// Check for and apply updates to the `codewhale` binary (merged from CLI).
...
async fn main() -> Result<()> {
    // Windows Terminal relocation must happen before any TUI setup so
    // plain-double-click users land in a proper terminal.
    relocate_to_windows_terminal();
    ...
    // Force-link the Dream consolidation prompt so the compiled-in
    // `DREAM_PROMPT` bytes survive MSVC LTO dead-code elimination.
    // `std::hint::black_box` prevents the optimizer from discarding the
    // otherwise-unreferenced static.
    std::hint::black_box(&prompts::DREAM_PROMPT);
    ...
    if let Some(command) = cli.command.clone() {
        return match command {
            ...
            Commands::Exec(args) => {
                ...
                let yolo = cli.yolo || config.yolo.unwrap_or(true);
            ...
            // ── CLI direct commands ──────────────────────────────
            Commands::Login { api_key, provider } => {
                let mut store = codewhale_config::ConfigStore::load(cli.config.clone())?;
                cli_commands::run_login_command(&mut store, provider, api_key)
            }
            Commands::Logout => {
                let mut store = codewhale_config::ConfigStore::load(cli.config.clone())?;
            ...
            Commands::Model(args) => cli_commands::run_model_command(args.command),
            Commands::Thread(args) => cli_commands::run_thread_command(args.command),
            Commands::AppServer(args) => cli_commands::run_app_server_command(args),
            Commands::Metrics(args) => cli_commands::run_metrics_command(args),
            Commands::Update(args) => cli_commands::run_update_command(args),
        };
    }
...
async fn run_doctor(config: &Config, workspace: &Path, config_path_override: Option<&Path>) {
    ...
    if crate::settings::detected_legacy_windows_console_host() {
        println!(
            "  {} legacy Windows console host → low_motion + synchronized_output=off (auto); fancy_animations not forced off",
            "•".truecolor(sky_r, sky_g, sky_b)
        );
        any_quirk = true;
    }
...
fn default_mouse_capture_enabled(
    ...
    if cfg!(windows) {
        ...
        // 已极少。如果出现 SGR 乱码可通过 `--no-mouse-capture` 或
        // `[tui] mouse_capture = false` 关闭。
        return true;
    }
