...
impl Settings {
    ...
    pub fn path() -> Result<PathBuf> {
        ...
        let primary = config_dir.join("codewhale").join("settings.toml");
        if primary.exists() {
            return Ok(primary);
        }
        let legacy = config_dir.join("deepseek").join("settings.toml");
        if legacy.exists() {
            // Auto-migrate: create codewhale dir, copy, return primary.
    ...
    pub fn set(&mut self, key: &str, value: &str) -> Result<()> {
        match key {
            ...
            "theme" | "ui_theme" | "background_color" | "background" | "bg" => {
                anyhow::bail!(
                    "Cannot change locked setting '{key}'. This build has theme and background_color pinned."
                );
            }
    ...
    pub fn available_settings() -> Vec<(&'static str, &'static str)> {
        vec![
            ...
            (
                "background_color",
                "Main TUI background color: #RRGGBB, transparent, or default",
            ),
            (
                "composer_density",
...
fn normalize_settings_theme(_value: &str) -> &'static str {
    "macaron"
}
...
fn normalize_background_color_setting(value: &str) -> Result<Option<String>> {
    ...
    if trimmed.eq_ignore_ascii_case("transparent") {
        return Ok(Some("transparent".to_string()));
    }

    normalize_hex_rgb_color(trimmed).map(Some).ok_or_else(|| {
        anyhow::anyhow!(
            "Failed to update setting: invalid background_color '{value}'. Expected #RRGGBB, RRGGBB, transparent, or default."
        )
    })
}
