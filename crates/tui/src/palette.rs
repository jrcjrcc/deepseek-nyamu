...
pub const ACCENT_REASONING_LIVE: Color = Color::Rgb(54, 187, 212);
...
/// Macaron dark — warm pastel macaron colors on dark chocolate.
pub const MACARON_UI_THEME: UiTheme = UiTheme {
    name: "macaron",
    mode: PaletteMode::Dark,
...
impl ThemeId {
    ...
    pub const fn tagline(self) -> &'static str {
        match self {
            ...
            Self::Macaron => "Warm pastel macaron on dark chocolate",
        }
    }
...
pub fn ui_theme_from_settings(theme: &str, background_color: Option<&str>) -> UiTheme {
    ...
    if let Some(bg) = background_color {
        if bg.trim().eq_ignore_ascii_case("transparent") {
            ui_theme = ui_theme.with_background_color(Color::Reset);
        } else if let Some(color) = parse_hex_rgb_color(bg) {
            ui_theme = ui_theme.with_background_color(color);
        }
    }
    ui_theme
}
...
fn nearest_ansi16(r: u8, g: u8, b: u8) -> Color {
    ...
    } else if u16::from(r) + 48 >= u16::from(b) && u16::from(r) > u16::from(g) + 24 {
        if bright {
            Color::LightMagenta
        } else {
            Color::Magenta
        }
    } else if u16::from(g) + 48 >= u16::from(b) && u16::from(g) > u16::from(r) + 24 {
        if bright {
            Color::LightCyan
        } else {
            Color::Cyan
...
mod tests {
    ...
    #[test]
    fn ui_theme_applies_custom_background_to_all_surfaces() {
        ...
        assert_eq!(theme.panel_bg, custom);
        assert_eq!(theme.elevated_bg, custom);
        assert_eq!(theme.composer_bg, custom);
    ...
    fn ui_theme_from_settings_applies_theme_and_background() {
        ...
        // with_background_color now sets ALL bg fields to the override color.
        assert_eq!(theme.surface_bg, Color::Rgb(17, 17, 17));
        assert_eq!(theme.panel_bg, Color::Rgb(17, 17, 17));
        assert_eq!(theme.elevated_bg, Color::Rgb(17, 17, 17));
        assert_eq!(theme.composer_bg, Color::Rgb(17, 17, 17));
