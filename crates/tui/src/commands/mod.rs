...
/// 根据 CommandInfo 的 usage 字段返回命令前缀（& 或 /）
pub fn cmd_display_prefix(cmd: &CommandInfo) -> &'static str {
    if cmd.usage.starts_with('&') { "&" } else { "/" }
}
...
pub const COMMANDS: &[CommandInfo] = &[
    ...
    CommandInfo {
        name: "glance",
        aliases: &["lan", "gailan", "速览"],
        usage: "&glance",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "search",
        aliases: &["sousuo", "查找", "chazhao"],
        usage: "&search <关键词>",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
    ...
    CommandInfo {
        name: "format",
        aliases: &["geshihua", "格式化"],
        usage: "&format",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "scaffold",
        aliases: &["jiaoshoujia", "脚手架"],
        usage: "&scaffold <名称>",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
    ...
    CommandInfo {
        name: "implement",
        aliases: &["daoru", "shixian", "实现"],
        usage: "&implement <URL|路径>",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "refactor",
    ...
    CommandInfo {
        name: "security-scan",
        aliases: &["securityscan", "安全扫描"],
        usage: "&security-scan [路径]",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "predict-issues",
        aliases: &["predictissues", "yuce", "预测"],
        usage: "&predict-issues",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "remove-comments",
        aliases: &["removecomments", "清理注释"],
        usage: "&remove-comments [文件]",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "fix-imports",
        aliases: &["fiximports", "xiufudaoru", "修复导入"],
        usage: "&fix-imports",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "find-todos",
        aliases: &["findtodos", "查找待办"],
        usage: "&find-todos",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "create-todos",
        aliases: &["createtodos", "添加待办"],
        usage: "&create-todos",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "fix-todos",
        aliases: &["fixtodos", "shixiantodo", "实现待办"],
        usage: "&fix-todos [文件]",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
    ...
    CommandInfo {
        name: "contributing",
        aliases: &["gongxian", "贡献分析"],
        usage: "&contributing",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "make-it-pretty",
        aliases: &["makeitpretty", "meihua", "美化"],
        usage: "&make-it-pretty [范围]",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "session-start",
        aliases: &["sessionstart", "kaishihuihua", "开始会话"],
        usage: "&session-start",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "session-end",
        aliases: &["sessionend", "jieshuhuihua", "结束会话"],
        usage: "&session-end",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
    ...
    CommandInfo {
        ...
        aliases: &["todostoissues", "todozhuanissue", "待办转议题"],
        usage: "&todos-to-issues",
        description_id: MessageId::CmdHelpDescription,
    },
    CommandInfo {
        name: "dream",
...
pub fn execute(cmd: &str, app: &mut App) -> CommandResult {
    ...
    if raw.starts_with('&') {
        ...
        if AMPERSAND_COMMANDS.contains(&cmd_name.as_str()) {
            return dispatch_ampersand_command(&cmd_name, arg, app);
        }

        return CommandResult::error(format!(
            "未知命令：&{cmd_name}。自定义命令以 & 开头，内置命令以 / 开头。输入 /help 查看可用命令。"
        ));
    }
    ...
    match command.as_str() {
        ...
        // Project commands
        "init" | "initiate" | "chushihua" | "初始化" => init_wizard::run_interactive_wizard(app),
            ...
            if suggestions.is_empty() {
                ...
                let list = suggestions
                    ...
                    .map(|name| {
                        ...
                        let prefix = info.map_or("/", |c| cmd_display_prefix(c));
                        format!("{prefix}{name}")
                    })
                    .collect::<Vec<_>>()
                    .join(", ");
                CommandResult::error(format!(
                    "未知命令：/{command}。您是不是想输：{list}？输入 /help 查看可用命令。"
                ))
            }
        }
    }
...
/// 调度 & 前缀的自定义命令
fn dispatch_ampersand_command(name: &str, arg: Option<&str>, app: &mut App) -> CommandResult {
    match name {
        // 项目管理
        "init" | "initiate" | "chushihua" | "初始化" => init_wizard::run_interactive_wizard(app),
        "glance" | "lan" | "gailan" | "速览" => glance::glance(app),
        "search" | "sousuo" | "查找" | "chazhao" => search::search(app, arg),
        // "mark" | "biaoji" | "标记" => mark::mark(app, arg),  // 已删除
        "plan" | "guihua" | "规划" => plan_cmd::plan(app, arg),
        "commit" | "tijiao" | "提交" => commit_cmd::commit(app, arg),
        "cleanproject" | "clean" | "qingli" | "清理" => clean_cmd::cleanproject(app, arg),
        // CCPlugins 系列
        "format" | "geshihua" | "格式化" => plugins::format(app, arg),
        "scaffold" | "jiaoshoujia" | "脚手架" => plugins::scaffold(app, arg),
        "test" | "ceshi" | "测试" => plugins::test(app, arg),
        "implement" | "daoru" | "shixian" | "实现" => plugins::implement(app, arg),
        "refactor" | "chonggou" | "重构" => plugins::refactor(app, arg),
...
pub fn all_command_names_matching(
    ...
    let mut result: Vec<String> = COMMANDS
        ...
        .map(|cmd| format!("{}{}", cmd_display_prefix(cmd), cmd.name))
        .collect();
