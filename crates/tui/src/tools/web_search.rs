...
/// No built-in default key. Users must set `[search] api_key` in config.toml
/// or the `METASO_API_KEY` environment variable. Rate-limited keys are available
/// at https://metaso.cn/search-api/playground (free tier: ~100 searches/day).
/// Previously shipped with a hardcoded community key; removed for security.
...
impl WebSearchTool {
    ...
    /// Search via Metaso AI Search API (<https://metaso.cn>). Requires
    /// `[search] api_key` in config.toml or `METASO_API_KEY` env var.
    /// Returns a clear error if neither is set (no built-in default key).
    async fn run_metaso_search(
        ...
        let api_key = context
            ...
            .ok_or_else(|| {
                ToolError::execution_failed(
                    "Metaso search requires an API key. \
                     Set `[search] api_key` in config.toml, or the METASO_API_KEY \
                     env var. Get one at https://metaso.cn/search-api/playground"
                        .to_string(),
                )
            })?;
...
mod tests {
    ...
    #[tokio::test]
    async fn metaso_provider_requires_api_key_when_not_configured() {
        // The built-in default key was removed for security. Without a config
        // key or env var, the provider MUST return a missing-key error.
        use crate::config::SearchProvider;
        use crate::tools::spec::{ToolContext, ToolSpec};
        ...
        assert!(
            ...
            "should refuse search when no Metaso API key is configured; got `{msg}`"
        );
    }
}
