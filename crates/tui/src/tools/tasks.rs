...
impl ToolSpec for TaskGateRunTool {
    ...
    async fn execute(&self, input: Value, context: &ToolContext) -> Result<ToolResult, ToolError> {
        ...
        if command.len() > 100_000 {
            return Ok(ToolResult::error(
                "BLOCKED: command exceeds 100 KB maximum length".to_string(),
            )
            .with_metadata(json!({
                "safety_level": "blocked",
        ...
        // Critical-danger gate: even in YOLO mode, block commands that
        // could destroy the system (root-filesystem rm, fork bomb, find /
        // -delete, pipe remote content to shell). These have no legitimate
        // use in a verification gate tool.
        if is_critically_dangerous(&command) {
            return Ok(ToolResult::error(
                "BLOCKED: gate command classified as critically dangerous — \
                 rm -rf /, fork bombs, find / -delete, and remote pipe-to-shell \
                 are never valid gate commands."
                    .to_string(),
            )
            .with_metadata(json!({
...
/// Check if a command is critically dangerous — patterns that would destroy
/// the system or execute arbitrary remote code. These are hard-blocked even
/// in YOLO mode for `task_gate_run` because they have no legitimate use as
/// a verification gate command.
fn is_critically_dangerous(command: &str) -> bool {
    ...
    let find_root = lower.contains("find /") && lower.contains("-delete");
    // Pipe remote content directly to shell
    let remote_pipe = (lower.contains("curl") || lower.contains("wget"))
        && (lower.contains("| sh") || lower.contains("| bash") || lower.contains("| zsh"));
    // Shell eval as primary command
    let eval_primary = {
        let trimmed = command.trim();
        trimmed == "eval" || trimmed.starts_with("eval ") || trimmed.starts_with("eval$")
    };

    rm_root || fork_bomb || find_root || remote_pipe || eval_primary
}
