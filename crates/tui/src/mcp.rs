...
impl McpTransport for StdioTransport {
    ...
    /// If the child does not exit within the grace window, force-kill it and
    /// wait again so the child is properly reaped (no zombie on Unix).
    /// `kill_on_drop` in tokio's Drop sends SIGKILL but does *not* wait —
    /// we do the final wait here.
    async fn shutdown(&mut self) {
        send_sigterm(&self.child);
        // Give the child a window to exit cleanly after SIGTERM.
        if tokio::time::timeout(STDIO_SHUTDOWN_GRACE, self.child.wait())
            .await
            .is_err()
        {
            // Grace period expired; force-kill and wait to reap the child.
            // If the child already exited between the timeout and the kill
            // call, `kill()` is a no-op (ESRCH) and `wait()` returns
            // immediately — both are safe to ignore.
...
impl McpConnection {
    ...
    pub async fn connect_with_policy(
        ...
        let transport: Box<dyn McpTransport> = if let Some(url) = &config.url {
            ...
            // Wrap the synchronous spawn in an async block so it can be
            // passed to `tokio::time::timeout` for a startup deadline.
            ...
            .with_context(|| {
                format!(
                    "MCP server '{name}' spawn timed out after {connect_timeout_secs}s \
            ...
            // Health check: if the child exited immediately (crash on startup),
            // surface the exit status and captured stderr before we try to
            // initialize the protocol handshake.
            if let Some(status) = child.try_wait()? {
                anyhow::bail!(
                    "MCP server '{name}' exited immediately with status {status} \
                     (transport=stdio cmd={command:?} args={:?})",
                    config.args,
                );
            }
