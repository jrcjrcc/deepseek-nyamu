...
impl Config {
    ...
    pub fn deepseek_api_key(&self) -> Result<String> {
        ...
        if matches!(provider, ApiProvider::Deepseek | ApiProvider::DeepseekCN)
            ...
            && !configured.starts_with("YOUR_")
        {
            return Ok(configured.clone());
        }
        // Detect placeholder key from config.example.toml
        if let Some(configured) = self.api_key.as_ref()
            && configured.starts_with("YOUR_")
        {
            bail!(
                "API key appears to be a placeholder ('{configured}'). \
                 Replace it with a real key in config.toml or set the DEEPSEEK_API_KEY \
                 environment variable. See config.example.toml for reference."
            );
        }
        ...
        // Catch placeholder keys in provider-scoped config too.
        if let Some(configured) = self
            .provider_config_for(provider)
        ...
        {
            bail!(
                "API key for '{provider:?}' in config.toml appears to be a placeholder ('{configured}'). \
