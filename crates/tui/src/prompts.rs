...
/// Dream memory-consolidation instructions — compiled-in so a fresh
/// deployment doesn't need a separate `dream-prompt.md` file on disk.
/// Pass this as the prompt when opening a sub-agent for Dream
/// consolidation (with `allowed_tools: [read_file, write_file,
/// edit_file, grep_files, list_dir]`).
///
/// Uses `include_bytes!` + `pub const` like every other prompt constant.
/// `std::hint::black_box` in the startup call (main.rs) prevents the
/// MSVC linker from stripping the payload via LTO.
pub const DREAM_PROMPT: &[u8] = include_bytes!("prompts/dream.md");
...
mod tests {
    ...
    #[test]
    fn dream_prompt_is_embedded() {
        // DREAM_PROMPT must be non-empty and carry the key phase markers.
        // ensure_dream_prompt_embedded() is called at startup in main.rs
        // to force the MSVC linker to retain the payload.
        let content = std::str::from_utf8(DREAM_PROMPT).expect("DREAM_PROMPT valid utf-8");
        assert!(!content.is_empty(), "DREAM_PROMPT should not be empty");
        assert!(
            content.contains("Orient"),
            "DREAM_PROMPT must contain the Orient phase"
        );
        assert!(
            content.contains("Gather"),
            "DREAM_PROMPT must contain the Gather phase"
        );
        assert!(
            content.contains("Consolidate"),
            "DREAM_PROMPT must contain the Consolidate phase"
        );
        assert!(
            content.contains("Prune"),
            "DREAM_PROMPT must contain the Prune phase"
        );
        assert!(
            content.contains("521KB"),
            "DREAM_PROMPT must specify 521KB total limit"
        );
        assert!(
            content.contains("tips.md"),
            "DREAM_PROMPT must reference protected tips.md"
        );
        assert!(
            content.contains("traps.md"),
            "DREAM_PROMPT must reference protected traps.md"
        );
    }
}
