...
fn main() {
    // Windows 图标资源
    embed_resource::compile("codewhale.rc", embed_resource::NONE);
...
fn configure_windows_stack() {
    ...
    match std::env::var("CARGO_CFG_TARGET_ENV").as_deref() {
        Ok("msvc") => {
            println!("cargo:rustc-link-arg-bin=codewhale=/STACK:8388608");
            println!("cargo:rustc-link-arg-bin=deepseek-tui=/STACK:8388608");
        }
        Ok("gnu") => {
            println!("cargo:rustc-link-arg-bin=codewhale=-Wl,--stack,8388608");
