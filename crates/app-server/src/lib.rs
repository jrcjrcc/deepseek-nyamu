...
fn resolve_auth_token(options: &AppServerOptions) -> Result<Option<String>> {
    ...
    if options.auth_token.is_some() {
        ...
        // Print only a masked prefix to avoid leaking the full token
        // into stderr logs or process-dump output. The full token is
        // available via the return value for programmatic use.
        // When CODEWHALE_APP_SERVER_SHOW_TOKEN=1, print the full token.
        if std::env::var("CODEWHALE_APP_SERVER_SHOW_TOKEN").as_deref() == Ok("1") {
            eprintln!("app-server auth: generated bearer token for this process.");
            eprintln!("  Authorization: Bearer {token}");
        } else {
            let prefix: &str = &token[..token.len().min(12)];
            eprintln!("app-server auth: generated bearer token (prefix: {prefix}…).");
        }
