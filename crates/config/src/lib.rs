...
/// it already exists. When the primary path does not exist but the
/// legacy `~/.deepseek/` counterpart does and the subdirectory is
/// NOT a UI/appearance directory (`settings`, `themes`), migrate it
/// on first access (create the primary dir, copy contents, then
/// return primary).
///
/// UI subdirectories are intentionally skipped so old theme colors
/// do not override the new default UI. Users upgrading from
/// `deepseek-tui` get fresh UI defaults while keeping their sessions,
/// skills, and cache under `~/.codewhale/`.
pub fn resolve_state_dir(subdir: &str) -> Result<PathBuf> {
    let primary_home = codewhale_home()?;
    let primary = primary_home.join(subdir);
    if primary.exists() {
        return Ok(primary);
    }
    // UI appearance directories must NOT be migrated — let fresh
    // defaults apply so the new theme colors take effect.
    let is_ui_dir = subdir == "settings" || subdir == "themes";
    let legacy = legacy_deepseek_home()?.join(subdir);
    if legacy.exists() && !is_ui_dir {
        // Auto-migrate: create primary, copy contents, return primary.
        if let Err(e) = copy_dir_all(&legacy, &primary) {
            tracing::warn!(
                "auto-migrate {}/ failed: {e}; falling back to legacy path",
                subdir,
            );
            return Ok(legacy);
        }
        tracing::info!(
            "auto-migrated {}/ from {} to {}",
            subdir,
            legacy.display(),
            primary.display()
        );
...
/// Subdirectories under `~/.deepseek/` that should be migrated to
/// `~/.codewhale/` when a user upgrades from the legacy name.
///
/// UI theme/appearance directories (`settings`, `themes`) are intentionally
/// excluded so old color/theme configuration does not override the new
/// default UI. Users upgrading from `deepseek-tui` get fresh UI defaults
/// while keeping their sessions, skills, and cache.
const MIGRATE_SUBDIRS: &[&str] = &["sessions", "skills", "cache"];

/// Recursively copy a directory tree from `src` to `dst`.
/// Creates `dst` if it doesn't exist; overwrites files in `dst`.
...
/// v0.8.44+: one-time migration from `~/.deepseek/` to `~/.codewhale/`.
/// Copies the config file AND state subdirectories (sessions, settings,
/// skills, cache, themes) so users upgrading from the old binary name keep
/// all their data. Never overwrites an existing primary config.
pub fn migrate_config_if_needed() -> Result<()> {
    let primary_home = codewhale_home()?;
    let primary = primary_home.join(CONFIG_FILE_NAME);
    if primary.exists() {
        return Ok(());
    }
    let legacy_home = legacy_deepseek_home()?;
    let legacy_config = legacy_home.join(CONFIG_FILE_NAME);
    if !legacy_config.exists() {
        return Ok(());
    }
    // Copy the config to the new home.
    std::fs::create_dir_all(&primary_home)
        .context("failed to create codewhale config directory")?;
    std::fs::copy(&legacy_config, &primary)
    ...
    // Write a fresh tui.toml (~/.codewhale/tui.toml) so TuiPrefs::path()
    // uses the primary path instead of falling back to the legacy
    // ~/.deepseek/tui.toml. An empty file with #[serde(default)] gives
    // all-default UI preferences (theme = "dark"), which means the new
    // palette constants from the compiled binary take full effect.
    ...
    // Migrate state subdirectories (sessions, skills, cache).
    for subdir in MIGRATE_SUBDIRS {
        let src = legacy_home.join(subdir);
